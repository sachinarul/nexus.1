        ...
        <div className="h-full overflow-y-auto pr-2 custom-scrollbar">
            <div className="flex items-center justify-between mb-6 sticky top-0 bg-navy/95 backdrop-blur-md z-10 py-2 border-b border-white/5">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-white font-bold text-sm">@nexus_noble</h3>
                        <p className="text-gray-400 text-xs">Posts & Reels</p>
                    </div>
                </div>
                <a href="https://www.instagram.com/nexus_noble?igsh=ZWJwc3Zvdmc0NmRw" target="_blank" rel="noopener noreferrer" className="text-xs text-teal hover:text-white transition-colors border border-teal/20 px-3 py-1 rounded-full hover:bg-teal/10">Follow</a>
            </div>

            <div className="grid grid-cols-2 gap-3">
                {instagramPosts.map((post) => (
                    <div key={post.id} className="group relative aspect-[9/16] bg-white/5 rounded-lg overflow-hidden cursor-pointer">
            ...
            <div className="mt-4 text-center">
                <a href="https://www.instagram.com/nexus_noble?igsh=ZWJwc3Zvdmc0NmRw" target="_blank" className="text-xs text-gray-500 hover:text-white transition-colors">View full profile on Instagram →</a>
            </div>
        </div>
    );
};
