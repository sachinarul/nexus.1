...
const LinkedInFeed = () => {
    return (
        <div className="h-full overflow-y-auto pr-2 custom-scrollbar">
            <div className="flex items-center justify-between mb-6 sticky top-0 bg-navy/95 backdrop-blur-md z-10 py-2 border-b border-white/5">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-white font-bold text-sm">Noble Nexus</h3>
                        <p className="text-gray-400 text-xs">2,400 followers</p>
                    </div>
                </div>
            ...
            <div className="space-y-4">
                {linkedInPosts.map((post) => (
                    <div key={post.id} className="bg-white/5 rounded-xl border border-white/5 p-4 hover:border-white/10 transition-colors">
                        <div className="flex gap-3 mb-3">
                            <div className="w-10 h-10 rounded bg-gradient-to-br from-teal to-purple"></div>
                            <div>
                                <h4 className="text-white text-sm font-bold">{post.author}</h4>
                                <p className="text-gray-500 text-[10px]">{post.role}</p>
                        ...
                        {post.image && (
                            <img src={post.image} alt="Update" className="w-full rounded-lg mb-3 border border-white/5" />
                        )}

                        <div className="flex items-center gap-2 text-xs text-gray-500 pt-3 border-t border-white/5">
                            <div className="flex -space-x-1">
                                <div className="w-4 h-4 rounded-full bg-blue-500 border border-navy"></div>
                                <div className="w-4 h-4 rounded-full bg-green-500 border border-navy"></div>
                                <div className="w-4 h-4 rounded-full bg-red-500 border border-navy"></div>
                            </div>
            ...
            <div className="mt-4 text-center">
                <a href="https://www.linkedin.com/company/noblenexus-ie/" target="_blank" className="text-xs text-gray-500 hover:text-white transition-colors">View all updates on LinkedIn →</a>
            </div>
        </div>
    );
};
